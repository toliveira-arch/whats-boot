...
import { runAsSystem } from '@whats-boot/database';
...
export const registerController = handler(async (req, res) => {
  const result = await runAsSystem(() => authService.register(req.body, metaFrom(req)));
  setRefreshCookie(res, result.refreshToken);
  res.status(201).json({ accessToken: result.accessToken, user: result.user });
});

export const loginController = handler(async (req, res) => {
  const result = await runAsSystem(() => authService.login(req.body, metaFrom(req)));
  setRefreshCookie(res, result.refreshToken);
...
export const refreshController = handler(async (req, res) => {
  ...
  try {
    const result = await runAsSystem(() => authService.refresh(token, metaFrom(req)));
    setRefreshCookie(res, result.refreshToken);
...
export const logoutController = handler(async (req, res) => {
  const token = req.cookies?.[REFRESH_COOKIE] as string | undefined;
  await runAsSystem(() => authService.logout(token));
  clearRefreshCookie(res);
  res.json({ ok: true });
});
...
export const forgotPasswordController = handler(async (req, res) => {
  const result = await runAsSystem(() => authService.forgotPassword(req.body));
  res.json({
...
export const resetPasswordController = handler(async (req, res) => {
  await runAsSystem(() => authService.resetPassword(req.body));
